...
class LeaveReportTimeoff(models.Model):
    _name = "hr.leave.report.timeoff"
    _description = 'Timeoff Summary / Report'
    _auto = False
    ...
    def init(self):
        tools.drop_view_if_exists(self._cr, 'hr_leave_report_timeoff')

        self._cr.execute("""
            CREATE or REPLACE view hr_leave_report_timeoff as (
                ...
                leaves.active as active,
                abs(leaves.number_of_days) as number_of_days,
    ...
    def action_time_off_analysis(self):
        ...
        return {
            'name': _('Timeoff Report'),
            'type': 'ir.actions.act_window',
            'res_model': 'hr.leave.report.timeoff',
            'view_mode': 'tree,pivot,form',
            'search_view_id': [self.env.ref('timeoff_custom.view_hr_holidays_filter_report_timeoff').id],
            'domain': domain,
            'context': {
    ...
    def action_open_record(self):
        ...
        return {
            ...
            'res_model': 'hr.leave' if self.leave_id else 'hr.leave.timeoff',
        }
